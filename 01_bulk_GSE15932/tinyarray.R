...
boxplot(exp) # 检查数据分布
saveRDS(exp, paste0(geoID, "normalized_exp.rds"))

# 表型信息
pd = pData(gse)
pd = pd[c(9:16,25:32),]
pd$geo_accession # 核对样本名，与exp一致
glimpse(pd)
group_list=ifelse(str_detect(pd$title,"diabetes"), # 自行寻找分组信息所在的组合分组的情况
             "diabetes",
             "Control")
group_list = factor(group_list,levels = c("Control", "diabetes"))
group_list
# Group = data.frame(
#   sample = colnames(exp),
#   Group = group_list
...
# saveRDS(ids, paste0(gse@annotation, "_ids.rds"))
